...
#==================================================================================================================
    ...
    if drawpink:
        drawp = euler(in2[3:].replace("^","**"),in1[3:].replace("^","**"),x1_,x2_,2000,0.1,wWidth) 
        drawpa = euler(in2[3:].replace("^","**"),in1[3:].replace("^","**"),x1_,x2_,2000,-0.1,wWidth)
        drawp = alignwithcenter(drawp,xStart,yStart,mashtab)
        drawpa = alignwithcenter(drawpa,xStart,yStart,mashtab)
    
    draw1 = euler(constf1,constf2,x1_,x2_,2000,0.1,wWidth) 
    draw1a = euler(constf1,constf2,x1_,x2_,2000,-0.1,wWidth)    
    draw2 = euler(constx1,constx2,x1_,x2_,2000,0.1,wWidth)    
    draw2a = euler(constx1,constx2,x1_,x2_,2000,-0.1,wWidth)    
    draw3 = euler(f1x1,f2x2,x1_,x2_,2000,0.1,wWidth)    
    draw3a = euler(f1x1,f2x2,x1_,x2_,2000,-0.1,wWidth)    
    draw4 = euler(enc(f1x1)+"*"+enc(const_koef1),enc(f2x2)+"*"+enc(const_koef2),x1_,x2_,2000,0.1,wWidth)
    draw4a = euler(enc(f1x1)+"*"+enc(const_koef1),enc(f2x2)+"*"+enc(const_koef2),x1_,x2_,2000,-0.1,wWidth)
    draw9 = euler(enc(constx1)+"*"+enc(const_koef1),enc(constx2)+"*"+enc(const_koef2),x1_,x2_,2000,0.1,wWidth)    
    draw9a = euler(enc(constx1)+"*"+enc(const_koef1),enc(constx2)+"*"+enc(const_koef2),x1_,x2_,2000,-0.1,wWidth) 
    
    x1_ = x1_*(-1)
    x2_ = x2_*(-1)
    
    if drawpink:
        drawp1 = euler(in2[3:].replace("^","**"),in1[3:].replace("^","**"),x1_,x2_,2000,0.1,wWidth) 
        drawp1a = euler(in2[3:].replace("^","**"),in1[3:].replace("^","**"),x1_,x2_,2000,-0.1,wWidth)
        drawp1 = alignwithcenter(drawp1,xStart,yStart,mashtab)
        drawp1a = alignwithcenter(drawp1a,xStart,yStart,mashtab)
    
    draw5 = euler(constf1,constf2,x1_,x2_,2000,0.1,wWidth)    
    draw5a = euler(constf1,constf2,x1_,x2_,2000,-0.1,wWidth)    
    draw6 = euler(constx1,constx2,x1_,x2_,2000,0.1,wWidth)    
    draw6a = euler(constx1,constx2,x1_,x2_,2000,-0.1,wWidth)    
    draw7 = euler(f1x1,f2x2,x1_,x2_,2000,0.1,wWidth)    
    draw7a = euler(f1x1,f2x2,x1_,x2_,2000,-0.1,wWidth)    
    draw8 = euler(enc(f1x1)+"*"+enc(const_koef1),enc(f2x2)+"*"+enc(const_koef2),x1_,x2_,2000,0.1,wWidth)   
    draw8a = euler(enc(f1x1)+"*"+enc(const_koef1),enc(f2x2)+"*"+enc(const_koef2),x1_,x2_,2000,-0.1,wWidth)   
    draw10 = euler(enc(constx1)+"*"+enc(const_koef1),enc(constx2)+"*"+enc(const_koef2),x1_,x2_,2000,0.1,wWidth)    
    draw10a = euler(enc(constx1)+"*"+enc(const_koef1),enc(constx2)+"*"+enc(const_koef2),x1_,x2_,2000,-0.1,wWidth) 
