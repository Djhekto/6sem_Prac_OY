...
#    list_for_matrA = [elem for elem in Matr_R_const]
#    list_for_matrB = [elem for elem in Matr_R_const]
#    enumerate_lomaet = 0
#    for _ in Matr_R_const:
#        for ind,symb in symbols:
#            list_for_matrA[enumerate_lomaet] = list_for_matrA[enumerate_lomaet].replace(symb,ppp_a[ind])
#            list_for_matrB[enumerate_lomaet] = list_for_matrB[enumerate_lomaet].replace(symb,ppp_b[ind])
#        enumerate_lomaet+=1
    
    listfromMatrRconst = list(Matr_R_const)
    Matr_R_a = Matrix(4,4,listfromMatrRconst)
    Matr_R_b = Matrix(4,4,listfromMatrRconst)
    print(Matr_R_a,"QWLELQWELKQWL'ELQWKEL;QKW")
    
    for i,_ in enumerate(Matr_R_a):
        for ind,symb in symbols:
            Matr_R_a[i] = str(Matr_R_a[i]).replace(symb,enc(ppp_a[ind]))
        Matr_R_a[i] = str(Matr_R_a[i]).replace("t",enc(a))
    
    for i,_ in enumerate(Matr_R_b):
        for ind,symb in symbols:
            Matr_R_b[i] = str(Matr_R_b[i]).replace(symb,enc(ppp_b[ind]))
        Matr_R_b[i] = str(Matr_R_b[i]).replace("t",enc(b))
